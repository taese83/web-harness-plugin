...
`src/app/routes/Routes.tsx`와 각 페이지 파일은 직접 생성하지 않는다. 라우팅 코드가 80줄을 넘으면 문서 본문이 아니라 `routes.code.tsx`(분할 시 `layout-spec/routes.code.tsx`)로 분리하고 본문에는 경로만 남긴다. `developer`가 Phase 3에서 생성한다.
