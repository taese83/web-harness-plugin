...
- `_workspace/01_plan/requirements.md`, `feature-plan.md`, `tech-stack.md`, `plan-review.md`(우선 결정만)
...
7. 각 matrix 행은 `developer`가 구현 근거로, `next-contract-verifier`가 판정 근거로 그대로 사용할 수 있어야 한다 — 행이 없는 route/handler/env/cache는 구현 대상이 아니다.
...
- 문서만으로 `developer`가 추가 질문 없이 boundary/authz/cache 구현을 시작할 수 있다.
