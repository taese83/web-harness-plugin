...
- **`INJECTION_SUSPECT` 소비** (`_workspace/.contracts/skills/web-orchestrator/references/untrusted-content-quarantine.md`): 외부 콘텐츠가 실행에 들어오는 경로에서 ① 지시형 패턴 탐지가 **구현돼 있는지** ② 적중 항목이 신뢰 경계를 넘지 못하는지 ③ 기록된 마커 목록을 확인한다. 탐지 코드가 아예 없으면 "마커 0건"을 안전으로 읽지 않는다 — 그건 **미구현**이며 `FAIL`(owner: `developer`)이다
...
- 가드가 **있는지**만이 아니라 **순서**를 본다: 한도가 인증 뒤에만 있으면 401로 끝난 요청이 계측되지 않아
  자격증명 추측이 무제한이다(HIGH). 인증 전 진입 한도와 인증 후 주체 한도는 별개이며, 진입 한도의 key를
  클라이언트가 보낸 헤더(`x-forwarded-for` 등)에서 검증 없이 뽑으면 헤더 교체로 우회한다(HIGH) —
  배포 대상이 그 헤더를 덮어쓴다는 근거가 없으면 경로 단위 총량이 맞는 backstop이다.
