...
- **`INJECTION_SUSPECT` 소비** (`web-harness-read skills/web-orchestrator/references/untrusted-content-quarantine.md`): 외부 콘텐츠가 실행에 들어오는 경로에서 ① 지시형 패턴 탐지가 **구현돼 있는지** ② 적중 항목이 신뢰 경계를 넘지 못하는지 ③ 기록된 마커 목록을 확인한다. 탐지 코드가 아예 없으면 "마커 0건"을 안전으로 읽지 않는다 — 그건 **미구현**이며 `FAIL`(owner: `developer`)이다
