...
- versioned artifact/release policy가 있고 version/changelog 변경이 아직 필요하면 HANDOFF를 만들지 않는다. `version-analyzer` → `changelog-writer` → `version-file-updater`를 실행한 뒤 quality runner와 모든 영향받은 QA/manifest를 다시 생성하게 한다. 신규 private 앱은 자동 bump하지 않는다
...
## 지금 바로 실행하기
```bash
web-harness-script run-package-operation --project . --operation install
pnpm dev
...
```bash
web-harness-script preview-server --project .   # http://localhost:4173
```
`design-review.md`의 Preview Approval(승인 SHA-256·통과 TC)을 함께 링크한다.

## 실제 API 연동하기 (/api-connect)
...
3. src/mocks/ 폴더는 개발 환경에서만 활성화되므로 프로덕션에 영향 없음
...
## 디자인 수정하기 (/component-gen)
- 테마 변경: src/app/theme.ts 수정
...
## 프로젝트 구조
[locked profile에 맞는 App Router 또는 FSD 구조 트리]

## 주요 파일 위치
| 역할 | 파일 |
| API 설정 | profile별 실제 server/client API module |
...
## 배포 CI 설정하기 (deploy-ci-writer)
- 배포 타겟이 결정됐으면 `deploy-ci-writer`를 실행해 GitHub Actions workflow를 생성한다
...
4. `changelog-writer`와 `version-file-updater` 적용 후 현재 release 시도를 중단한다. 변경은 기존 receipt/manifest를 stale로 만든다
