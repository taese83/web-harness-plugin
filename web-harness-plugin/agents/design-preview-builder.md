...
12. 생성 직후 오케스트레이터가 `web-harness-script validate-design-preview --project {root} --write-source-snapshot`을 실행한다. 이 명령이 입력 스펙 SHA-256을 `traceability.json.sourceSnapshot`에 기록하며, builder가 digest를 추측하거나 직접 작성하지 않는다. 검증 실패 시 완료로 보고하지 않는다.
