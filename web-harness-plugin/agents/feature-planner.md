...
1. `requirements.md`(Product Frame 포함)와 `ux-brief.md`를 읽고 사용자 여정을 수직 FEAT로 나눈다. `paths=`는 스택이
   FSD면 `_workspace/.contracts/skills/component-gen/references/fsd-rules.md` 결정 트리를 따르고(한 페이지만 쓰는 기능은 그 페이지
...
4. timeseries 요구가 있으면 `time-range`, `chart-panel`, `live-mode`, `stream-status` 책임을 분리하고 historical query와
   ...
   모델링하지 않는다.
5. `LOCAL_DOMAIN_STATE_MODE`이면 `_workspace/.contracts/skills/web-orchestrator/references/local-domain-state.md`를 읽고
...
11. `_workspace/.contracts/skills/web-plan/references/design-readiness-contract.md`의 Page Groups와 Feature List 표준 표를 필수로
...
12. **서피스 모델을 근거가 있을 때만 선언한다.** 화면 단위를 무엇으로 구분하는가이며 값은 `route`(URL 경로 — 기본값)와
    ...
    `overlay`는 Phase 2까지만 성립하므로(정의·한계는 `_workspace/.contracts/agents/layout-designer.md`「서피스 모델」) Phase 1 → 2
...
13. **경량 재호출(write-back)**: 기능 추가·변경으로 재호출되면 Feature List의 대상 행과 관련 절만 현재화한다
    (`_workspace/.contracts/skills/web-plan/references/plan-history-contract.md`).
