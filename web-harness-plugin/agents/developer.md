...
- **디자인 근거 없이 화면의 조건을 지어내지 않는다.** 빈 상태·오류·권한 없음처럼 조건에 따라
  ...
  무엇이 없어서 멈췄는지 이름으로 보고한다(`PAGE-002[variant=권한 없음]`). 오케스트레이터가
  `--design-debt`로 그 자리를 이미 물었어야 하며(`phase-3-development.md` 「디자인 부채 청구」), 여기까지 온
  것은 그 청구가 놓친 것이다. 지어내면 **아무도 그것이 결정이었다는 사실을 모른다** — 이것이
  ...
  **멈추지 않는 두 경우**: ⓐ 정보 위계 표에 내용이 있으면 그것이 근거다 — 그리는 방식은
  ...
  그 ID가 `_workspace/01_plan/decision-log.md`(또는 분할된 `decision-log/`)에 **표제로**
  (`## PC-007`·`## D-042` 등 — 접두는 그 프로젝트의 로그가 정한다) 실재해야 한다.
  **맨 ID는 인수가 아니다** — 이 표에는 내용 근거로 ID를 인용하는 정상 용법이 있다.
  없는 ID를 적은 자리도 인수가 아니다(`--design-debt`가 그것을 따로 낸다) — 그 자리는 ⓑ가
  아니므로 **멈춘다**.

  **표가 없거나 분모가 깨진 경로는 예외다.** `no-plan`(기획 문서 없음)·`denominator-broken`
  (정보 위계 표를 읽지 못함)에서는 토큰을 적을 행 자체가 없다. 그때의 인수 정본은
  `decision-log`의 **범위를 명시한 엔트리**다 — "이 프로젝트의 화면 조건 전부"처럼 범위가
  적혀 있으면 그것이 ⓑ이며, 로그를 직접 읽어 판단한다(그 상태에서는 `--design-debt`가 분모를
  세지 못해 `acknowledged`를 낼 수 없다). 이 예외가 없으면 사용자가 문서화된 절차
  (`phase-3-development.md` ③)를 그대로 따랐는데도 멈춘다 — 인수 형식을 토큰 하나로 좁힌 것의
  대가이며, 교차 모델 리뷰(2026-09-04)가 회귀로 잡았다.

  **인수가 유효할 때만** 정하고 무엇을 정했는지 보고한다 —
