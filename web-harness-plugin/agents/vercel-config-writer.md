...
9. install은 `pnpm install --frozen-lockfile --ignore-scripts`, build는 일반 project에서 `pnpm run build`, static external ingestion에서 `web-harness-script run-vercel-static-ingestion-build`로 명시한다. typed wrapper가 semantic validation → reviewed argv build → source 불변성·semantic 재검증 → `public`/`dist|out` digest parity → 전체 output inventory와 quiescence 재검사를 수행한다. crawler/generate를 provider build에 넣지 않는다. 단, 이 wrapper만으로 detached child를 격리했다고 판단하지 않는다.
