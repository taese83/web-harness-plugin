...
- **결과가 중요한 후처리**: 응답 전에 **DB에 작업 레코드를 커밋**하고(outbox 패턴), 실행은 scheduled job이 가져간다 — `_workspace/.contracts/skills/web-orchestrator/references/background-jobs-contract.md`의 계약을 따른다.
