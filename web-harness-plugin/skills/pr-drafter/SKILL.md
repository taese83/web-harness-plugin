...
2. 확정한 base로 컨텍스트를 수집한다 (`{base}`는 1단계 결과):
   ```bash
   web-harness-script run-git-inspection --project . --operation log --base {base}
   web-harness-script run-git-inspection --project . --operation diff-stat --base {base}
   web-harness-script run-git-inspection --project . --operation diff --base {base}
   web-harness-script run-git-inspection --project . --operation status
   web-harness-script run-git-inspection --project . --operation diff
   ```
3. 변경 범위를 파악한다:
