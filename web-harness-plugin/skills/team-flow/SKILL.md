...
각 모드의 실행은 `web-harness-script ticket/cli <cmd>`가 담당한다(결과 JSON, 게이트 차단
...
| "이 Jira 티켓 읽어줘", "사람이 쓴 티켓으로 시작", "티켓에서 기획 가져와" | `intake <티켓키>`(역방향 입구) |
| "이 기획 티켓이 이 FEAT의 출처야" | `bind <FEAT> <기획 티켓키>`(출처 링크) |
| "내가 쓴 이 개발 티켓 하네스가 알게 해줘" | `adopt <FEAT> <개발 티켓키>`(청구 인수) |

**티켓 종류마다 문이 다르다** — 기획 티켓은 `intake`→`bind`(출처), 개발 티켓은 `claim` 또는
`adopt`(청구). 판정 기준과 각 문이 남기는 것은 `references/ticket-kinds.md`가 정본이다.
