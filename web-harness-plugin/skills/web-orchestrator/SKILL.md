...
`TIMESERIES_MODE`이면 일반 intake 뒤 `_workspace/.contracts/skills/timeseries-dashboard/references/intake-and-slos.md`의 질문을 규모·SLO 단계에서 최대 3개씩 추가한다. 사용자가 모르는 값은 baseline을 제안하고 `ASSUMPTION`으로 requirements에 전달한다.
