...
시작 전에 다음 공통 계약을 읽는다. **시점 표기가 있는 항목은 선행 로드하지 않고 그 시점 직전에 읽는다** — 고정 컨텍스트 오버헤드 절감이며, 시점 표기가 곧 계약이다.

- `references/interaction-contract.md`, `../web-plan/references/planning-facilitation-contract.md`, `../web-plan/references/planning-readiness-contract.md`
- `references/request-type-contract.md`, `references/scenario-contract.md`
- 실행 예산·telemetry·동적 확장은 `references/execution-budget-contract.md` — 첫 스폰 전에 읽고, **스폰이 끝날 때마다 결과 usage를 `_workspace/04_qa/execution-telemetry.json`에 기록한다** (usage 미제공 환경이면 `null`, 지어내지 않는다)
- 전 구간 금지·선행 조건은 `references/operational-gotchas.md`
...
`web-harness-read skills/ai-app-orchestrator/references/detection-contract.md`를 AI 기능의 단일 판별 기준으로 사용한다. 모델 생성, RAG, model-selected tool, 코드리뷰, AI 고객센터, AI analytics, browser action이 핵심이면 `AI_MODE: true`와 모든 submode를 기록하고 `/ai-app-orchestrator`의 설계·runtime·QA gate를 적용한다. 일반 검색, 고정 규칙, read-only Playwright QA는 제외한다.

`web-harness-read skills/web-orchestrator/references/local-domain-state.md`를 browser-owned domain state의 단일 판별 기준으로 사용한다. localStorage/IndexedDB/offline CRUD 또는 정렬·이동·다중선택·참조 불변식이 있으면 `LOCAL_DOMAIN_STATE_MODE: true`로 기록한다. 단순 theme/language persistence만 있으면 제외한다.
...
`TIMESERIES_MODE`이면 일반 intake 뒤 `web-harness-read skills/timeseries-dashboard/references/intake-and-slos.md`의 질문을 규모·SLO 단계에서 최대 3개씩 추가한다. 사용자가 모르는 값은 baseline을 제안하고 `ASSUMPTION`으로 requirements에 전달한다.
...
**Wave 2** — 단독 실행 (design-system.md + layout-spec.md 존재 후):
...
5. `TIMESERIES_MODE`이면 `mock-api-builder`를 실행해 완성된 `TimeseriesTransport` interface 기반 fake를 만든다.
   - browser Mock 사용 시 `public/mockServiceWorker.js`를 확인한다. dependency install이 승인·완료됐는데 파일이 없으면 실제 외부 격리가 적용된 setup job에서만 `WEB_HARNESS_ISOLATED_EXECUTION=1 web-harness-script run-package-operation --project {project-root} --operation msw-init`을 실행한다. 사용자 승인만 있는 host 실행은 `BLOCKED`다
...
- `AI_MODE` 정적 gate: _(저장소 모드 전용 단계 — 플러그인 배포판에서는 생략한다.)_
...
```bash
web-harness-script prepare-quality-attestation --project . --issuer-run-id <trusted-ci-run-id>
...
web-harness-script validate-release-gate --write-manifest
web-harness-script validate-release-gate
```
