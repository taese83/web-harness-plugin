...
```bash
web-harness-script init-workspace --project-root {project-root}
```
...
4. **여전히 별도인 것** — 소유 영역이나 실행 조건이 구별되는 것만 남긴다:
   - `developer`. `TIMESERIES_MODE`에서는 realtime interface 완료 후로 미룬다 <!-- marker:timeseries-realtime-build-order -->
...
5. browser Mock 사용 시 `public/mockServiceWorker.js`를 확인한다. dependency install이 승인·완료됐는데 파일이 없으면 실제 외부 격리가 적용된 setup job에서만 `WEB_HARNESS_ISOLATED_EXECUTION=1 web-harness-script run-package-operation --project {project-root} --operation msw-init`을 실행한다. 사용자 승인만 있는 host 실행은 `BLOCKED`다
