...
요구사항으로 재수집하는 순환이 된다. 개발 착수는 계획이 WORK로 발행한 티켓만 `pickup`한다.
...
기획 티켓은 **출처로 남는다** — 인벤토리 행과 스냅샷이 받은 기록이고, 어느 FEAT가 그것을 썼는지는 인벤토리의
「소비 지점」 열이 적는다. 티켓 본문에 마커를 찍어 FEAT와 묶는 `bind`는 FEAT 개발 티켓 경로와 함께
제거됐다(2026-09-14) — 개발은 계획이 발행한 WORK 티켓으로만 나간다(`team-flow`).
