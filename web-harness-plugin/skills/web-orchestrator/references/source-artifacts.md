...
```bash
web-harness-script ticket/cli intake <티켓키> --repo <owner/name>
```
...
### 컴포넌트가 분류를 정할 수 있다 — 팀이 선언했을 때만

트래커의 컴포넌트가 이미 축을 나누고 있으면 그 매핑을 설정에 선언한다:

```json
{"provider": "jira", "jira": {"componentAxis": {
  "PLAN": "기획 입력", "DESIGN": "디자인 입력", "DEVELOP": "개발 티켓"}}}
```

**어휘를 하네스가 정하지 않는다.** `PLAN`이 기획이고 `DEVELOP`이 아니라는 것은 그 팀의
약속이고, 코드에 박으면 특정 조직의 사고모델을 인코딩하는 것이다(I3). 매핑에 없는 컴포넌트는
**추측하지 않고** `미분류`로 둔다.

`개발 티켓`으로 선언된 컴포넌트는 **인테이크가 거부한다** — 그것은 파이프라인의 출력이지
입력이 아니다. 하네스가 발행한 티켓을 다시 기획 입력으로 들이면 자기가 만든 요구사항을
요구사항으로 재수집하는 순환이 된다. 그 티켓을 착수하려면 `pickup`이다.

**분류는 인테이크가 정하지 않는다.** 티켓이 기획인지 버그인지 운영 요청인지는 본문을 읽어야
알고, 애초에 이분법도 아니다(버그 티켓이 요구사항을 담기도 한다). 인테이크는 근거만 싣는다 —
트래커가 선언한 타입과 라벨. 인벤토리의 「분류」 열은 `미분류`로 들어가고
**`source-artifact-ingestor`가 스냅샷을 읽고 정한다**(기획 입력 / 디자인 입력 / 참고).
운영자가 미리 알면 `--as '참고'`로 명시할 수 있고, 그 경우 계약이 정한 셋 밖의 값은 거부된다.

FEAT가 만들어지면 묶는다:

```bash
web-harness-script ticket/cli bind <FEAT-NNN> <티켓키> --repo <owner/name>
```

원장에 청구를 기록하고 본문에 왕복 마커를 스탬프한다 — 그때부터 픽업이 그 티켓을 알아본다.
**근거 없이 묶지 않는다**: 그 티켓이 인벤토리에 있어야 하고, FEAT가 로컬 계획에 실재해야 하며,
양쪽 다 다른 곳에 묶여 있지 않아야 한다. 본문은 **덮어쓰지 않고 덧붙인다**(`stampRefsInto`).
