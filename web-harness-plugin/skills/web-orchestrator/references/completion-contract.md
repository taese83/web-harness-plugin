...
- 디자인 프리뷰가 있으면 그 위치(`_workspace/02_design/preview/`)와 재기동 명령(`web-harness-script preview-server --project {root}`), `APPROVED|STALE` 상태 확인 명령, FEAT/TC 추적 커버리지와 승인된 test case — 개발과 분리된 보존 자산이므로 완료 후에도 고객이 언제든 재확인할 수 있음을 명시
- 적용된 mode와 아직 Mock/ASSUMPTION인 경계
- 후속 명령: `/api-connect`, `/component-gen`, `/feature-add`, `/timeseries-dashboard`, `/analytics-chart-builder`, `/ai-app-orchestrator`, `/ai-eval`
