...
- **`autocomplete`로 자동 채움을 연다** — 이름·이메일·전화·주소·`username`·`current-password`·`new-password`·`one-time-code` (WCAG 1.3.5 AA). 같은 과정에서 이미 받은 정보를 다시 입력하게 하지 않는다 — 자동 채움이나 선택으로 대신한다(3.3.7).
- **숫자처럼 보이는 문자열은 `type="number"`가 아니다** — OTP·카드·우편번호는 `inputmode="numeric"`(JSX는 `inputMode`)(앞자리 0·자동 증감·지수 표기 문제 회피).
- **붙여넣기를 막지 않는다** — 비밀번호·OTP에서 붙여넣기나 비밀번호 관리자를 막으면 인증 접근성 위반이다(3.3.8). 분할 OTP 입력은 붙여넣은 값을 칸마다 나눠 채운다.
- **저장 안 한 변경이 있으면 이탈 전에 알린다** — 라우트 이동은 router blocker, 탭 닫기는 `beforeunload`. 저장 뒤에는 해제한다.
