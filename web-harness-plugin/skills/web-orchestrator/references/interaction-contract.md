...
- 답을 모르는 사용자에겐 측정 가능한 baseline을 제안하되 확정값처럼 말하지 않는다.
...
## 묻지 않는 것 — 사용자가 답할 근거가 없는 질문

**사용자는 이 답을 어디서 얻는가.** 산출물·코드·설정에 이미 적혀 있으면 읽어라. "하네스
아티팩트"가 답이면 묻지 마라 — 판정 책임만 넘기고 근거는 주지 않는 것이다.

**아티팩트끼리 어긋나면 결함이지 선택지가 아니다.** 정본은 소유권·모듈경계·레이어=`spec.json`,
스폰 범위=`change-scope.md` 기계 블록, 기술 방향=`tech-stack.md`, 수용 기준=`feature-plan`,
기존 관례=실측이며 **나중에 확정된 것이 앞선 계획을 이긴다**. 못 가르면 선택지가 아니라
`BLOCKED`(무엇이 어긋났나·무엇을 하면 풀리나)다. 사용자가 답하려면 잠금 시각·digest·소유권
같은 내부 사정을 알아야 하는 질문은 애초에 묻지 않는다.
