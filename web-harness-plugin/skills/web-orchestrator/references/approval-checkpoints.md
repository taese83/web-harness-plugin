...
```bash
web-harness-script validate-handoff-readiness --project {root} --motion-role
```
...
기준은 이미 있다: 개발자가 픽업 미리보기에서 확인한 판정서의 완료 조건과 `TT-` 테스트 항목이다(「기획·디자인 `absent`
...
- 사용자가 그것도 원하지 않으면(티켓 작업이 아닐 때) `unverifiable`을 유지할 수 있다. 그때는 이 문서의
