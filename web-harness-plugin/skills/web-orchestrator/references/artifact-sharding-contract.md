...
```bash
web-harness-script validate-artifact-sharding --project {project-root}
```
...
- 절 파일·INDEX·미분할 단일 파일이 예산을 넘지 않는가 (절 개수 트리거 포함 — 단
  **분할 금지 문서**는 절 개수 트리거를 적용하지 않고, KB 초과 시 "분할"이 아니라 "축소"
  ...
  - `02_design/solution-design.md` — `spec.mjs`의 `lockSpec`이 flat 경로를 요구하므로
    ...
    상한(15KB)을 가진다** — 블록도 `rationale`·`openDecisions.options` 같은 자유 문장을
    담으므로 "줄일 수 없는 것"이 아니다. 상한을 넘으면 그 문장들을 줄인다
  `solution-design.md`의 **결정 블록(`web-harness:solution-design`)만 예산에서 뺀다.** 그
  블록은 `spec.mjs`가 파싱해 읽는 계약이고 저자가 산문처럼 줄일 대상이 아니다. 제외는 세 겹으로
  좁다 — ① 이 경로일 것 ② 라벨이 정확히 그것일 것 ③ JSON으로 파싱될 것. 하나라도 어긋나면
  **산문으로 센다**. 라벨 와일드카드(`web-harness:*`)를 모든 파일에 적용하는 안은 거부됐다:
  아무 라벨이나 붙여 산문을 감싸면 예산이 사라진다(2026-09-02 실측 45,007B → 7B). 제외되는
  블록에도 **자체 상한 15KB**가 있다(복수 블록이면 합산) — 유효 JSON 문자열에 산문을 넣는
  잔여 경로를 유계로 만든다. **이 절이 측정 규칙의 정본이다** — validator 주석과 §4 등록부는
  참조·한계만 적는다. 반증 seed 6종과 긍정 seed 2종이 이 경계를 고정한다.
