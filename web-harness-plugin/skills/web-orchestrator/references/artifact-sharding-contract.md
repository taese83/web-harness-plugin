...
- 절 파일·INDEX·미분할 단일 파일이 예산을 넘지 않는가 (절 개수 트리거 포함 — 단
  **분할 금지 문서**는 절 개수 트리거를 적용하지 않고, KB 초과 시 "분할"이 아니라 "축소"
  ...
  - `02_design/solution-design.md` — `spec.mjs`의 `lockSpec`이 flat 경로를 요구하므로
    분할하면 스팩 확정이 깨진다. 시정은 산문(rationale·alternatives) 축소이며, **기계 블록은
    줄이는 대상이 아니다**
  예산은 기계 블록을 포함해 **파일 전체 바이트**로 잰다. 블록을 예산에서 빼자는 제안이
  있었으나 거부됐다 — 이름 제한 없는 펜스는 산문을 담을 수 있어 예산을 통째로 우회한다
  (2026-09-02 실측: 45,007B → 7B). 회귀 seed가 이 우회를 고정한다.
