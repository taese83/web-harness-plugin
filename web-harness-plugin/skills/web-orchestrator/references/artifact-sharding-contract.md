...
설계 산출물 하나를 평균 5~13개 에이전트가 입력으로 읽는다. 단일 파일이 50KB를 넘으면 각 소비자가 자기 담당과 무관한 40KB 이상을 매번 함께 읽는다. 분할은 산출물의 정보량을 줄이는 것이 아니라 **읽는 단위를 담당 범위에 맞추는 것**이다.
...
`주 소비자` 열은 소비자가 자기 절을 고르는 근거다. 비워 두지 않는다.
...
- 각 절의 `주 소비자`가 실제 에이전트 이름인가 (warn)
