...
`certified`는 라벨이 아니라 증거다 — `supportLevel: certified`인
...
통과한다(`validate-certified-evidence`). 어떤 레인이든 T1 폐곡선을 CI에서 그린으로 재현하면 같은 게이트 아래에서 certified를 획득한다.
...
```bash
web-harness-script web-core/resolve-profile --project-root {project-root} --requested auto
```
...
```bash
web-harness-script web-core/resolve-profile --project-root {project-root} --requested {profile-id} --provider {generic|vercel} --deployment {target} [--capability {capability} ...]
...
- `next-app-fullstack`은 `_workspace/.contracts/skills/next-app/SKILL.md`로 위임하고 Vite 전용 `environment-scaffolder`, `developer`, Vite template을 호출하지 않는다.
