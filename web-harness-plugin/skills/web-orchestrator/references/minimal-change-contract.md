...
1. edit 전에 관련 call path, public export, test를 읽고 current status/diff는 `web-harness-script run-git-inspection --project {project-root} --operation <status|diff>`로 수집한다. 직접 `git`을 실행하지 않는다.
