...
3. **피드백**: 라운드당 최대 3개 항목. `ASSUMPTION(프리뷰 A/B)` 항목은 앱 내 토글로 비교해 확정한다. 동작이 test case와 다르면 그 자체가 피드백 항목이다.
...
- Phase 2뿐 아니라 `iterate` 모드·`/feature-add`에서도 프리뷰 루프에 재진입한다. 화면·동작이 바뀌면 스펙을 고치고 프리뷰를 재생성해 다시 확인한다.
...
프리뷰는 일회성 산출물이 아니라 **릴리스까지 보존되는 자산**이다. 승인 후에도 언제든 고객에게 다시 보여줄 수 있어야 한다.
