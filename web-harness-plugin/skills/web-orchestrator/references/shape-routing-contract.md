...
| `serverless-functions` | (개발 착수 직전) `api-schema-designer` | `_workspace/.contracts/skills/vite-serverless-hybrid/SKILL.md` 계약의 `api/` handler | `api-contract-verifier` |
...
| client/server 계약 | `communication` + `api-schema.md`의 `API_CONTRACT`가 `none`이 아님 | `_workspace/.contracts/skills/api-contract-typegen/SKILL.md` |
