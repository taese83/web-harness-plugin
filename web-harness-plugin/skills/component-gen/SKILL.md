...
6. **QA — 신규 생성 및 기존 컴포넌트 수정 후 반드시 실행한다** (생략 불가, 사용자가 묻기 전에 자동 실행):
   ...
   **6-2. 결정론적 quality 실행**:
   - 사용자 승인 후 `web-harness-script run-quality-gates --all --allow-host-execution`
...
- source 변경으로 기존 receipt와 qa-manifest가 stale이므로 release 전 `_workspace/.contracts/skills/web-verify/SKILL.md`의 절차로 다시 검증한다.
