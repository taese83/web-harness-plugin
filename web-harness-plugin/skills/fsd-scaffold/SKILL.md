...
Read `references/fsd-rules.md` before any layer decision. Read `references/slice-template.md` when generating file scaffolding. 기존 프로젝트에 파일을 생성하거나 연결할 때는 `web-harness-read skills/web-orchestrator/references/minimal-change-contract.md`를 읽고 새 slice에 필요한 integration path만 수정한다.
