...
Read `references/checklist.md` before starting any phase. 템플릿 catalog 전체를 읽지 않는다. 파일을 만들기 직전에 `web-harness-script read-skill-section --catalog project-templates --section <SECTION>`을 실행해 필요한 section 하나만 가져온다. section 이름이 불명확할 때만 `--list`를 사용한다.
