...
`visual-contract-designer`는 `visual-qa-contract.json` 하나를 만든다(같은 내용의 마크다운 사본은 만들지 않는다). JSON은 `web-harness-read schemas/visual-qa-contract.schema.json`을 따른다.
