...
`visual-contract-designer`는 `visual-qa-contract.json` 하나를 만든다(같은 내용의 마크다운 사본은 만들지 않는다). JSON은 `_workspace/.contracts/schemas/visual-qa-contract.schema.json`을 따른다.
...
공급된 디자인이 있으면 reference의 상류 정본은 `_workspace/00_source/design-binding.json`이다(`_workspace/.contracts/skills/web-orchestrator/references/design-binding-contract.md`). 그 파일이 어느 화면의 어느 조건에 어느 근거가 붙는지를 이미 선언했으므로 여기서 다시 정하지 않고 **같은 id로 같은 것을 가리킨다** — 어긋나면 `visual-evidence-lib`가 거부한다. 공급된 근거 전부를 target으로 올릴 필요는 없다(critical·brand·layout-risk 선택 기준이 우선). 요구되는 것은 일치이지 전수 채택이 아니다.
