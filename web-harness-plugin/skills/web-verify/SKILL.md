...
2. 테스트 기반을 준비한다:
   ...
   - developer
   - `VISUAL_QA_MODE`이면 visual-developer. baseline은 갱신하지 않는다.
...
9. locked profile이 `next-app-fullstack`이면 서명 완료 후 `web-harness-script web-core/validate-next-contracts --project {project-root}`를 실행한다. 이어서 `next-contract-verifier`를 실행하고 반환 본문을 `_workspace/04_qa/qa-next-contract.md`에 저장한다. 로컬 진단, target-specific receipt·artifact digest 또는 trusted attestation이 없으면 `BLOCKED`로 남긴다.
10. `_workspace/04_qa/`에 모든 결과를 저장한 뒤 `web-harness-script validate-release-gate --write-manifest`와 `web-harness-script validate-release-gate`를 실행한다. source, receipt 또는 trust configuration이 서명 후 바뀌면 quality runner부터 다시 시작한다.
11. release gate exit 0일 때만 release-manager가 일반 QA와 조건부 QA 리포트를 종합해 HANDOFF를 만든다. exit 0이 아니면 `web-harness-read skills/web-orchestrator/references/release-tier-contract.md`에 따라 gate error를 분류해 tier(`DIAGNOSTIC_VERIFIED`/`ISOLATED_VERIFIED`/`NOT_VERIFIED`)를 판정하고, release-manager가 `_workspace/RELEASE/release-readiness.md`에 tier·근거·승급 경로를 기록한다. tier 라벨을 상향 표현으로 바꾸지 않는다.
