...
기존 application/runtime source를 수정할 때는 `web-harness-read skills/web-orchestrator/references/minimal-change-contract.md`를 읽는다. service builder와 retry owner에게 `CHANGE_MODE`, `ALLOWED_PATHS`, 보존할 tool/data/public contract, `NON_GOALS`, `CHANGE_BUDGET`, test evidence를 전달하고 AI 기능 추가를 이유로 unrelated web/runtime 구조를 재작성하지 않는다.
