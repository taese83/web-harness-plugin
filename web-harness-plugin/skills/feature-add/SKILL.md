...
Read `web-harness-read skills/web-orchestrator/references/request-type-contract.md` before selecting the mini-cycle. Before the first source edit, read `web-harness-read skills/web-orchestrator/references/scenario-contract.md`, `web-harness-read skills/web-orchestrator/references/design-approval-contract.md`, `web-harness-read skills/web-orchestrator/references/minimal-change-contract.md`, `web-harness-read skills/web-orchestrator/references/integration-overlay.md`, `web-harness-read skills/web-orchestrator/references/change-journal-contract.md`, and `web-harness-read skills/web-orchestrator/references/development-gates-contract.md`.
...
- **시나리오**: `web-harness-read skills/web-orchestrator/references/scenario-contract.md`에서 관련 카테고리만 선택한 정상·실패·경계 동작
...
- 검증 명령은 receipt 래퍼로 실행해 기계 기록을 남긴다 (산문 "통과했다" 금지):
  ```bash
  web-harness-script record-verification --project {project-root} --label build \
...
- **04_qa receipt 재생성 (배포 대상 프로젝트, 생략 불가)**: `_workspace/04_qa/evidence/`가 있는 프로젝트에서는
  라운드 종료 전에 `web-harness-script run-quality-gates --project {project-root} --all`로 receipt를 재발급한다.
...
- **인증 뒤 화면**이면 `web-harness-read skills/web-orchestrator/references/auth-verification-contract.md`를 따른다 —
