...
- **실제 연결**: Mock transport가 완성된 뒤 `_workspace/.contracts/skills/api-connect/SKILL.md`와 이 skill의 migration contract를 함께 적용한다.
...
8. `developer`가 차트 UI와 historical/live 데이터를 연결한다(UI 범위와 데이터 범위는 별도 스폰).
