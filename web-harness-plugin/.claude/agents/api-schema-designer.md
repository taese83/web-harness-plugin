...
- `API_CONTRACT: provisional` — 실제 API·OpenAPI·공급 문서가 아직 없다. 화면 동작(FEAT·TC)과 Must AC에서 도출한 잠정
  계약이고, 개발은 이 계약으로 MSW mock을 쓴다(`.claude/skills/mock-service-setup/SKILL.md`). 잠정인 필드·규칙은 `ASSUMPTION`으로 표시한다.
...
`.claude/skills/api-contract-typegen/SKILL.md`로 스키마를 가져오고 `.claude/skills/api-connect/SKILL.md`로 Mock을 실제
endpoint에 연결한다.
...
`.claude/skills/web-orchestrator/references/artifact-sharding-contract.md`의 크기 예산과 분할 규칙을 따른다. **쓰기 전에 형태를 정한다** — 절(H2) 목록을 먼저 세워 계약의 절 수·크기 예산을 넘을 것 같으면 처음부터 `_workspace/02_design/api-schema/`에 쓴다(리소스별 절 + 공통 envelope·에러 절 1개 + `INDEX.md`). 단일 파일을 먼저 쓰고 나중에 나누지 않는다 — 둘이 함께 있으면 분할 검사가 거부한다. 기준은 리소스 수가 아니라 절 수다. `INDEX.md`는 표만 두고 5KB 이하로 쓰며, 백틱 파일명으로는 이 폴더의 절 파일만 적는다 — 분할 검사가 INDEX의 백틱 파일명을 절 파일로 대조하므로, 다른 산출물의 근거 인용은 절 본문에 둔다. 80줄을 넘는 Zod/MSW 코드 블록은 절 본문이 아니라 `{리소스}.code.ts`로 분리하고 본문에는 경로만 남긴다.
