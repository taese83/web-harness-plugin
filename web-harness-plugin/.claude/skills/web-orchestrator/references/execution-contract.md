...
1-A. `change` 레인이면 `approval-checkpoints.md`의 「change 레인 → 개발」(기획 개정 → 디자인 델타 감지 → 개정 → 스팩 → **✋승인**)을 수행한다(사람 티켓은 `specApproval: required`일 때만). **확인 전에는 source edit를 시작하지 않는다.** `fix`·`verify`는 건너뛴다. 승인 뒤에는 `development-gates-contract.md`의 **Gate 0**(개발 착수 준비)을 돌린다 — Iterate는 Phase 3을 거치지 않으므로 여기서 걸지 않으면 그 관문이 도달 불가다.
