...
`VISUAL_QA_MODE`이면 `.claude/skills/visual-design-verify/SKILL.md`의 target/state/mode와 baseline governance를 Phase 2 승인에 포함한다. prototype mode에서 rendered evidence 또는 visual contract가 없으면 `BLOCKED`다.
