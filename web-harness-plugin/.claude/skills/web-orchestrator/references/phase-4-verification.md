...
테스트 판정은 에이전트가 아니라 스크립트가 한다 — quality runner 뒤에 `node .claude/scripts/report-test-qa.mjs --project {root}`를 실행하면 영수증에서 판정을 계산해 `_workspace/04_qa/qa-test.md`와 판정 기록을 남긴다(옮겨 적지 않는다). 변이 표본도 함께 돈다.
