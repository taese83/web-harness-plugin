...
사용자 확인은 ①이 맡는다(§4). 세운 뒤 `node .claude/scripts/validate-artifact-sharding.mjs --project {root}`가 exit 0이어야
...
개발 중에 생긴 스팩 변경은 `phase-3-development.md` 「개발 중 스팩 변경」이 정본이다 — 이미 확정된
스팩을 바꾸는 요청이라 거부는 현행 유지, 보류는 그 범위 `BLOCKED`다(`assumed`로 닫지 않는다).
...
되돌리기 비용이 큰 결정(디렉토리 구조, 데이터 계층, 형태) · 잠정 API 계약(`API_CONTRACT: provisional`)으로 착수하는 것.
...
성격이 다르다 — 전자는 관측이라 무엇도 막지 않지만, 후자가 없으면 `developer`는 **아무것도
