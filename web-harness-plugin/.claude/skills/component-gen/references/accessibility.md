...
- **vendored 프리미티브(tailwind-shadcn 레인)**: 기반 프리미티브(Radix·Base UI)가 공급하는 a11y 구조(`aria-*`·`role`·Portal·focus trap)는 upstream-파생 계약이다 — 스타일·구성만 바꾸고 a11y 배선을 제거하지 않는다. 이탈 시 해당 줄에 한 줄 사유를 남긴다.
