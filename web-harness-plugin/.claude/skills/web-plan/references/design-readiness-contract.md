...
```markdown
## 화면별 정보 위계
| 화면 | info:Primary 정보 (1~3개, 순서 = 중요도) | info:Secondary | info:밀도 | state:empty | state:error | variant:권한 없음 |
|---|---|---|---|---|---|---|
| PAGE-001 | ① 금일 오류율 ② 응답시간 추이 | 서비스별 분포, 최근 배포 | 고밀도 | 온보딩 안내 + 데이터 연결 CTA | 마지막 정상 데이터 + 재시도 | 접근 요청 안내 |
```

- Primary는 "사용자가 이 화면에서 3초 안에 얻어야 하는 것" — 3개 초과면 화면 분할을 검토한다.
- **빈 상태 · 오류 · 권한은 기본으로 두고 시작한다.** 이 화면들에 해당이 없어도 **열을 지우지 말고** 칸에 `해당 없음(사유)`를 적는다 — 열을 지우면 그 조건은 분모에서 빠져 아무도 요구하지 않게 된다(2026-09-08 코퍼스 검증: 열을 지우는 것으로 분모를 3→2→1개까지 줄여도 게이트가 아무 말도 하지 않았다).
  - **필수는 아니다.** 서비스에 그 부류가 없으면 다른 축을 쓴다 — 실측에서 본인인증 서비스는 권한 대신 `variant:인증단계`·`variant:자격`을 썼다. `--to design` 인계는 빠진 축을 **보고만 하고 막지 않는다**.
- 상태별 내용은 컴포넌트가 아니라 **사용자에게 보여줄 내용**으로 쓴다 ("EmptyState 컴포넌트" ✗, "아직 데이터가 없다는 안내와 첫 연결 버튼" ✓).

### 이 표는 조건의 분모다 — 기계가 읽는 형식

하나의 화면은 조건에 따라 여러 디자인을 갖는다(권한 없음·빈 상태·모바일). 그 조건 목록을
세우는 것이 이 표이며, `design-binding-contract.md`가 그 목록을 분모로 커버리지를 판정한다.
읽히려면 세 가지가 필요하다.

1. **첫 열은 화면의 식별자다** — `PAGE-NNN`, 또는 Page Groups 표의 `Page`·`Route/Screen`과
   **정확히 일치하는** 문자열. 어느 쪽도 아니면 그 행은 어느 화면의 것인지 알 수 없다.
   **`PAGE-001 세미나 목록`처럼 ID 뒤에 라벨을 덧붙이는 것은 허용된다** — 사람이 읽기 좋으라고
   하는 정상 표기이며 ID가 앞에 있으면 모호하지 않다. 실측(2026-09-08 프로브)에서 계약을 따라
   돈 ingestor가 정확히 이 형태를 냈고, 종전 판정은 그 행을 전부 미해소로 잡아 **분모를 통째로
   붕괴**시켰다 — 잘 쓴 기획서가 「분모 없음」이 되는 것은 오탐이지 미결이 아니다.
2. **첫 열 뒤의 모든 헤더는 `형:이름` 형식이다.** 형은 넷뿐이다 — 조건은
   `state`·`modeId`·`variant`(`design-binding-contract.md` §2와 같은 어휘), 서술은 `info`.
   `info:밀도`처럼 조건이 아닌 열도 형을 붙인다.
   **형이 없는 열은 미결로 잡힌다.** 조건을 키워드로 추측하지 않기 때문이다 — `무료 플랜 시`
   같은 임의의 `variant`를 목록으로 알아맞히는 것은 완전할 수 없고, 알아맞히기에 실패하면
   그 조건이 조용히 분모에서 빠진다. 형을 요구하면 판정이 추측이 아니라 **전역 함수**가 된다.
   **`state:default`는 열로 두지 않는다** — Primary·Secondary가 그것을 서술하므로 표에 오른
   모든 화면이 기본 조건을 갖는 것으로 본다.
3. **데이터 행의 칸은 비우지 않는다.** 조건이 이 화면에 해당하지 않으면 `해당 없음(사유)`로
   **명시**한다. 그 화면은 그 조건의 분모에서 빠진다.
4. **`ack:<ID>` 토큰은 인수 기록이다.** 조건 칸에 `ack:PC-007`을 적으면 그 조건이, 이름 칸이나
   `info:` 칸에 적으면 그 행의 조건 전부가 **인수된 것으로 읽힌다**(`--design-debt`의
   `acknowledgedBy`). 토큰은 결정 로그와 대조되며 로그에 없는 ID는 인수가 아니라 오류로
   보고된다. **맨 ID(`PC-002`·`REQ-F-001`·`FEAT-003`)는 인수가 아니다** — 이 표에는 내용 근거로
   결정·요구 ID를 인용하는 정상 용법이 있고(`checkDecisionsLanded`가 그것을 요구한다), 둘을
   같은 토큰으로 읽으면 내용 인용이 부채를 지운다. 절차는 `phase-3-development.md`
   「디자인 부채 청구」다.
   토큰 접두 `ack:`와 ID는 **대소문자를 가리지 않고 읽되**(`Ack:pc-007`도 토큰이다) 대조는
   대문자로 정규화해 결정 로그 표제와 맞춘다 — 로그 표제 자체는 대문자 ID여야 한다.
   **`state:default`만 인수하는 표기는 없다.** 기본 조건은 열이 아니라 Primary·Secondary
   서술에서 오므로 토큰을 적을 칸이 없고, 이름·`info:` 칸에 적으면 그 행 전부가 인수된다.
   default만 인수하려면 다른 조건을 `해당 없음(사유)`로 명시해 분모에서 빼거나, 조건 칸마다
   따로 적는다 — 이 공백은 `docs/protected-core.md` §4에 등록돼 있다.

**빈 칸은 결정이 아니라 미결이다.** 종전에는 표의 **존재만** 검사돼서, 표를 만들고 상태 칸을
비워두면 통과했다 — 그러면 조건 분모가 0이 되어 커버리지가 `0/0`으로 서고, **조건을 안 적는
것이 통과하는 길**이 됐다(`docs/protected-core.md` §4에 등록됐던 항목). 지금은 빈 칸이
`NEEDS_DECISION`이며 인계 검사가 잡는다.

**행은 화면이지 상태가 아니다.** 상태를 행으로 나열한 표(단일 라우트 앱에서 흔하다)는 첫 열이
Page Groups로 해소되지 않아 미결로 잡힌다. 그 형태는 **화면 한 행 + 상태 열**로 옮긴다 —
상태가 행이면 한 화면의 조건 집합을 기계가 묶을 수 없다.

**한계(정직)** — 전부 `docs/protected-core.md` §4에 등록한다:

- 절 앵커(`화면별 정보 위계`·`Information hierarchy`)와 N/A 어휘(`해당 없음`·`비적용`·`n/a`·`—`)가
  **낱말에 결박돼 있다.** 다른 언어·다른 낱말로 적으면 절을 못 찾거나, 사유 있는 N/A가 조건으로
  세어져 근거를 요구한다. 산문 간선 대조와 같은 클래스다.
- 조건 **값**은 검사하지 않는다. 표가 `state:emty`로 오타를 내고 바인딩이 같은 오타를 쓰면
  덮인 것으로 센다. `variant`가 자유 문자열이어야 하는 이상(I3) 값을 고정할 수 없다.
- 암묵 기본 조건의 낱말이 `default`로 고정돼 있다. `visual-qa-contract.json`의 `state`에
  `idle` 같은 다른 낱말을 쓰는 프로젝트는 표에 없는 `default` 행을 적게 된다.

## 2. 디자인 방향 (ux-brief 필수 절)

인테이크에서 수집하고, 모르는 항목은 값을 지어내지 않고 `ASSUMPTION(시안 확정)`로 표기한다 — 발산 조사(design-principles-research.md)가 커밋한 단일 시안의 승인에서 확정된다.

```markdown
## 디자인 방향
- 브랜드 제약: (색/로고/폰트 지정 여부, 없으면 "없음")
- 참조 무드: (참조 서비스/화면과 "어떤 점"이 좋은지)
- 밀도: 고밀도(대시보드형) | 표준 | 여백 중심 — 미정이면 ASSUMPTION(시안 확정)
- 다크모드: 필수 | 선택 | 불필요
...
- `design-system-architect`: 디자인 방향 절이 없으면 `BLOCKER`. `ASSUMPTION(시안 확정)` 항목은 발산 조사(`design-principles-research.md`)로 방향을 비교한 뒤 **커밋한 단일 토큰**으로 준비하고 기각 방향·이유를 기록한다 — 토큰 변형 세트는 사용자가 비교를 명시 요청한 opt-in 축에만 만든다.
