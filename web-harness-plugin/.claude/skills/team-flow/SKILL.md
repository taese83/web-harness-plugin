...
**보드는 결과만 보여준다.** 행과 `next`, 하네스가 낸 `notes`를 그대로 옮긴다. 하네스가 말하지 않은
이유를 추측해 덧붙이거나(스팩·기획 상태를 따로 조사해 설명하지 않는다), 앞선 답을 정정하는 해설을
붙이지 않는다 — 티켓을 보여 달라는 요청에 필요한 것은 목록과 다음 할 일뿐이다.
