...
`--confirm`이면 **소비 메타데이터만** 새 판본에 맞춘다: 본문(소비 FEAT·책임 TC·마커)을 다시 쓰고, 라벨을 증감하고,
티켓에 코멘트로 알린 뒤 원장에 `publish-synced`를 남긴다.
...
- 이미 픽업한 작업이면 그 change-scope는 옛 판본이다 — `link`가 STALE로 막으므로 다시 픽업한다(코멘트가 그렇게 알린다).
