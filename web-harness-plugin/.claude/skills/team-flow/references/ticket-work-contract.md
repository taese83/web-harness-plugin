...
손으로 만들든 하네스가 만들든(`create`) 개발 티켓 본문은 네 절과 선택 절 둘이다 — 판정이 흔들리지 않게 같은 양식을 쓴다.
본문 순서는 목적 → 수정 범위 → 작업 내용 → 하지 않는 것 → 완료 조건 → 선행·협의이고, 선택 절은 쓸 때만 싣는다.

| 절 | 쓰는 법 | 판정서로 가는 곳 |
|---|---|---|
| 목적 | 이 작업이 이루는 것 한 문장. 아래에 `근거:`(기획 문서·페이지) 한 줄 — 또는 선행·협의 절 끝에 `상세기획:`·`디자인:` 링크 줄(초안에서도 그 절을 마지막에 쓴다) | `objective` |
| 수정 범위 (선택) | 고칠 경로 목록. 쓰면 작업 내용에서 경로를 되풀이하지 않는다 | `writePaths`의 근거 |
| 작업 내용 | 무엇을 어디에(경로) 만드는지 목록, 끝에 `하지 않는 것:` | `writePaths`의 근거 · `nonGoals` |
| 하지 않는 것 (선택) | 범위 밖 목록과 한 줄 이유. 쓰면 작업 내용 끝의 `하지 않는 것:`을 대신한다 | `nonGoals` |
...
`create`는 이 양식을 기계로 검사한다(네 절·빈 선택 절 없음·완료 조건 목록·FEAT/TC 없음·같은 제목의 열린 개발 티켓 없음). 팀이 손 티켓 제목에
붙이는 접두어는 트래커 설정 `titlePrefix`로 받아 같은 제목 대조에도 쓴다. 만든 티켓은
...
연결 전에 리뷰한다 — `link --dry-run`의 `review`(하네스 리뷰어 + 트래커 설정 `reviewAgents`로 팀이 선언한 프로젝트 리뷰어)를 같은 범위로 부른다.
CLI는 리뷰어를 알려 줄 뿐 리뷰가 돌았는지 증명하지 않는다.
