...
//
// 손으로 만든 개발 티켓과 **같은 것**을 만든다 — 네 절 양식(목적·작업 내용·완료 조건·선행·협의), 팀이 선언한
...
const SECTION_BY_TITLE = new Map(TICKET_SECTIONS.map(section => [section.title.replace(/\s+/g, ''), section.id]))
// 기획 요구사항(FEAT/TC)은 계획이 발행하는 WORK의 몫이다 — 개발 티켓이 그 ID를 달면 두 모델이 섞인다.
const PLAN_ID = /\b(?:FEAT|TC)-\d+(?:-\d+)?\b/
const LIST_ITEM = /^\s*[-*]\s+\S/

/**
 * 초안 markdown → 티켓 초안들(순수). `## 제목`이 티켓 하나, 그 아래 `### 절`이 네 절이다.
 ...
 */
export function validateTicketDrafts({drafts, openTickets = []}) {
  const errors = []
  const create = []
  const existing = []
  if (list(drafts).length === 0) errors.push('초안에 티켓이 없다 — `## 제목` 아래 네 절(목적·작업 내용·완료 조건·선행·협의)을 쓴다')
  const seen = new Set()
  ...
  for (const draft of list(drafts)) {
    ...
    if (list(draft.unknownSections).length > 0) errors.push(`${label}: 양식에 없는 절 ${draft.unknownSections.map(name => `「${name}」`).join(', ')} — 네 절 안에 적는다`)
...
/**
 * 본문(순수) — 네 절을 트래커 서식의 제목으로 잇는다. Jira 위키 서식은 목록 기호를 `*`로 바꾼다.
...
export function renderDevTicketBody(draft, {format = 'markdown'} = {}) {
  ...
  return TICKET_SECTIONS.map(({id, title}) => `${heading(title)}\n${block(draft.sections[id])}`).join('\n\n')
}
