...
/** units 로드 — --units <json파일> 우선, 없으면 로컬 feature-plan.md 파싱(표 형식이면 0 unit →
 * 하류 EMPTY_UNITS_CLOSE_ALL 가드가 잡음). 지어내지 않음. */
...
export async function runLink({root, featureId, prUrl, flags, io = {}}) {
  ...
  if (changeScope?.featureId === featureId) {
    ...
    if (isChangeScopeStale(changeScope, unit)) {
      return {ok: false, blocked: 'stale-change-scope', guidance: '픽업 후 상류 계획이 바뀌었습니다 — 계획 동기화·재확인 후 PR을 완료하세요'}
    }
  }
  ...
  if (plan.status === 'already-linked') return {ok: true, idempotent: true, existing: plan.existing, closeLine}
  if (!flags.confirm) return {ok: true, dryRun: true, closeLine, record: plan.record}
  ;(io.append ?? appendLedgerRecord)(ledgerFile, plan.record)
  return {ok: true, dryRun: false, closeLine, record: plan.record}
}
...
export async function runBoard({root, repo, developer, flags, io = {}}) {
  ...
  } catch { /* 트래커 조회 실패 → 로컬-only 보드(정직 강등, tracker: unavailable) */ }
  ...
  return {board, merged, tracker: issuesByFeature.size > 0 ? 'live' : 'unavailable'}
}
...
// dispatch가 오발화한다(실측).
const invokedDirectly = process.argv[1]?.split('/').at(-1) === 'cli.mjs'
if (invokedDirectly) {
