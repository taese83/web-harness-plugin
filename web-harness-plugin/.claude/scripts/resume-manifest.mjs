...
import {planLockLedgerPath, planDigest, readSpecAt, specDigestOf} from './validate-spawn-plan.mjs'
...
export function readLockLedger(manifestPath, {exists = existsSync} = {}) {
  const path = planLockLedgerPath(manifestPath)
...
export function inspectPlanSpecBinding(root, lock, manifestPath = null, spec = undefined) {
  const fromLedger = manifestPath
    ? readEvidenceLog(planLockLedgerPath(manifestPath))
