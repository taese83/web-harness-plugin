...
//
// 근거(telemetry 원본, seminar-booking 전 과정): 22스폰 중 미완 3건이 각각 130~170k를 스펙
// 재독에 쓰고 산출물 작성 전/중 종료했다(미완 소비 424k = 전체의 15%, 셋 다 재스폰으로 복구).
// **빈도는 낮지만 1회 비용이 크다** — 그래서 사후 복구보다 사전 차단이 싸다.
//
...
// 실측 소비는 130~170k였다 — 추정치가 그 위다. 게다가 추정은 선언
...
function main() {
  ...
  if (opts.lock && report.verdict === 'FITS') {
    ...
    const ledgerEntries = readEvidenceLog(planLockLedgerPath(planPath))
    ...
    appendEvidenceLine(planLockLedgerPath(planPath), {task: plan.task ?? null, digest, at, specDigest})
